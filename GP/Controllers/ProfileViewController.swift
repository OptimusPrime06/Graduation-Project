...
class ProfileViewController: UIViewController {
    ...
    private let nameLabel: UILabel = {
        ...
        label.textColor = .white // Adjust text color for contrast
        label.text = "Name"
        return label
    }()
    ...
    private let editEmailButton: UIButton = {
        ...
        button.setTitleColor(.white, for: .normal) // Adjust button color for contrast
        button.backgroundColor = UIColor.systemBlue.withAlphaComponent(0.8)
        button.layer.cornerRadius = 8
        button.addTarget(ProfileViewController.self, action: #selector(editEmailTapped), for: .touchUpInside)
        return button
    }()
    ...
    @objc private func editEmailTapped() {
        ...
        alert.addAction(UIAlertAction(title: "Save", style: .default) { [weak self] _ in
            ...
            guard let newEmail = alert.textFields?.first?.text,
                  !newEmail.isEmpty,
                  newEmail.isValidEmail() else { // Validate email format
